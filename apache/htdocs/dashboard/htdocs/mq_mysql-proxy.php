...
$title = "MQ Tier mysql-proxy-process Data - $graphDownSample Downsample";
 ...
 */

 {
     ...
     $tsd->addMetric("sum:$graphDownSample-avg:process.mon.rss_mb{process=mysql-proxy,app=king,host=*,cluster=mq}");
 ...
 
 {
     ...
     $tsd = new Tsd($graphTime);
     $tsd->addMetric("sum:$graphDownSample-avg:process.mon.vsize_mb{process=mysql-proxy,app=king,host=*,cluster=mq}");
     $template->addGraph($tsd->getDashboardHTML($graphWidth, $graphHeight), $graphName);
 }

 {
     ...
     $tsd = new Tsd($graphTime);
     $tsd->addMetric("sum:$graphDownSample-avg:process.mon.pcpu{process=mysql-proxy,app=king,host=*,cluster=mq}");
     $template->addGraph($tsd->getDashboardHTML($graphWidth, $graphHeight), $graphName);
 }

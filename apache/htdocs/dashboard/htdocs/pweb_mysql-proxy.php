...
$title = "Web Tier mysql-proxy-process Data - $graphDownSample Downsample";
 ...
 */

 {
     ...
     $tsd->addMetric("sum:$graphDownSample-avg:process.mon.rss_mb{process=mysql-proxy,app=*,host=*,cluster=pweb}");
 ...
 
 {
     ...
     $tsd = new Tsd($graphTime);
     $tsd->addMetric("sum:$graphDownSample-avg:process.mon.vsize_mb{process=mysql-proxy,app=*,host=*,cluster=pweb}");
     $template->addGraph($tsd->getDashboardHTML($graphWidth, $graphHeight), $graphName);
 }

 {
     ...
     $tsd = new Tsd($graphTime);
     $tsd->addMetric("sum:$graphDownSample-avg:process.mon.pcpu{process=mysql-proxy,app=*,host=*,cluster=pweb}");
     $template->addGraph($tsd->getDashboardHTML($graphWidth, $graphHeight), $graphName);
 }
